# This GUI allows a user to control the Multi Puffer Sampler System with button presses
#
...
volumes_feed = "volumes_feed_uriCheckout_2.csv" #only vols from the HBs
command_hist = "command_hist_uriCheckout_2.csv" #all commands sent + ACKs from sampler. May include some serial line errors from startup.
sampler_feed = "sampler_Feed_uriCheckout_2.csv" #everything received from sampler (HB's and ACK's)
...
class PumpFrame(ttk.Frame):
    def __init__(self, container, pumpInd):
        ...
        self.onCmd = b"MP Unit 0 Start_Pump " + str(self.pumpInd).encode() + b"\r"
        self.offCmd = b"MP Unit 0 Stop_Pump " + str(self.pumpInd).encode() + b"\r"
...
class OtherButtons(ttk.Frame):
    def __init__(self, container):
        ...
        # Clear EEPROM
        def clear():
            ser.write(b"MP __Set_SN OECI_MP_003\r") 
            print("Clear EEPROM")
            log(["MP __Set_SN OECI_MP_003"], command_hist)
        ...
        wakegSampler = tkinter.IntVar()
        wakeSampler_state=tkinter.Button(self, text="Wake", command=wake, borderwidth=6)
        wakeSampler_state.grid(row=0,column=1)
        ...
        # start mission
        def start_mission():
            ser.write(b"MP Start_Mission 0 0 0 0\r")
            print("Start Mission")
            log(["MP Start_Mission 0 0 0 0"], command_hist)
        ...
        # End Mission
        def end_mission():
            ser.write(b"MP End_Mission\r")
            print("End Mission")
            log(["MP End_Mission"], command_hist)
        ...
        # Calibrate pumps
        def calibrate():
            ser.write(b"MP Unit 0 Cal_Pump_All to 0.354\r") 
            print("Calibrate Pumps")
            log(["MP Unit 0 Cal_Pump_All to 0.354"], command_hist)
